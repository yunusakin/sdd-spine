...
- Target: ${tool_name}
- Bootstrap context: \`bash scripts/context-pack.sh --task bootstrap\`
- Intake context: \`bash scripts/context-pack.sh --task intake-core\`
- Verification gate: \`bash scripts/verify-work.sh\`
...
- Use \`bash scripts/context-pack.sh --task implementation-discuss\` before coding.
- Use \`bash scripts/verify-work.sh --scope app\` before marking implementation ready.
EOF
      cat <<EOF | write_file "${TARGET_ROOT}/.cursor/rules/spectra-context.mdc"
# Spectra Context Routing

- Ask Spectra for task-specific context with \`bash scripts/context-pack.sh --task <pack_id>\`.
...
- Run \`bash scripts/discuss-task.sh --item <id> --task-type <type> --goal "<goal>"\` before post-approval coding.
- Run \`bash scripts/verify-work.sh --scope app\` before handoff.
EOF
      cat <<EOF | write_file "${TARGET_ROOT}/.windsurf/rules/spectra-context.md"
# Spectra Context

- Resolve the required pack with \`bash scripts/context-pack.sh --task <pack_id>\`.
EOF
      ;;
    copilot)
...
- Use \`bash scripts/discuss-task.sh --item <id> --task-type <type> --goal "<goal>"\` before implementation work.
- Use \`bash scripts/verify-work.sh\` before marking work complete.
EOF
      cat <<EOF | write_file "${TARGET_ROOT}/.agent/rules/spectra-context.md"
# Spectra Context

- Resolve task context via \`bash scripts/context-pack.sh --task <pack_id>\`.
EOF
      ;;
    *)
